...
test_that("simple truncation works, negative strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncate5primeTxome(txdb_single_contig_w1000_neg, maxTxLength=n, quiet = T)
...
test_that("idempotent", {
  txdb_res_w500_1 <- truncate5primeTxome(txdb_single_contig_w1000, maxTxLength=500, quiet = T)
...
test_that("spliced truncation works, positive strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncate5primeTxome(txdb_multi_exon, maxTxLength=n, quiet = T)
...
test_that("spliced truncation works, negative strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncate5primeTxome(txdb_multi_exon_neg, maxTxLength=n, quiet = T)
